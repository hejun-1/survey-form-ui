...
class StepSurvey extends React.PureComponent {
  ...
  onUserIdChange(evt) {
    ...
    if (!evt.target.value) {
      ...
      service.validateUserId('自采尿液送检HIV问卷调查-北京信息科技大学', evt.target.value).then(() => {
        this.setState({
          validateStateClass: ''
        });
      }).catch(() => {
  ...
  isValidated() {
    ...
    return service.validateUserId('自采尿液送检HIV问卷调查-北京信息科技大学', this.props.getStore().userId).then(() => {
          ...
          <div>
            <div className="wizard-step-header">
              <label className="col-md-12 control-label">
                <h2>第{this.props.index}步: 填写调查问卷(北京信息科技大学)</h2>
              </label>
            </div>
            <div className="row">
              <div className="col-md-12">
                <div className="wizard-card wizard-scroll-container">
                  ...
                  <Form school={"北京信息科技大学"} showGroupCQuestions={false} ref={f => form = f} getStore={() => (this.props.getStore())} updateStore={(u) => {this.props.updateStore(u)}}></Form>
                </div>
              </div>
            </div>
          </div>
